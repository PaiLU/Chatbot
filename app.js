    ...
    function(session, args, next){
        ...
        http.request(options, function(res) {
            ...
            res.on('data', function (data) {
                ...
                session.endConversation("Your Employee ID: %s <br\>Name: %s<br\>Your remaining annual leaves: %s day(s)<br\>Your current pending leaves: %s day(s)", receive[0].id||receive.id, session.message.user.name, receive[0].annualLeave||receive.annualLeave, receive[0].pending||receive.pending||0);
            });
        }).end();
    }
])
